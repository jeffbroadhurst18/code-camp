    ...
    public class CampMappingProfile : Profile
    {
		public CampMappingProfile()
		{
			CreateMap<Camp, CampModel>()
				...
				camp.Length > 0 ?
				camp.EventDate.AddDays(camp.Length - 1) : camp.EventDate))
				.ForMember(c => c.Url,
			...
			CreateMap<Speaker, SpeakerModel>().ForMember(c=> c.Url, opt => opt.ResolveUsing<SpeakerUrlResolver>())
				.ReverseMap();

			CreateMap<Talk, TalkModel>().ForMember(c => c.Url, opt => opt.ResolveUsing<TalkUrlResolver>())
				.ReverseMap();
		}
    }
}
